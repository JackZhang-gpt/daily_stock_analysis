...
_SKILL_BASE_WEIGHTS: Dict[str, float] = {
    "bull_trend": 1.15,
...
class SkillAggregator:
    ...
    def _compute_weight(
        ...
        static_weight = _SKILL_BASE_WEIGHTS.get(skill_id, 1.0)
        if perf_weight is not None:
            return base_weight * static_weight * perf_weight
        return base_weight * static_weight * self._backtest_factor(opinion.agent_name, min_samples)
